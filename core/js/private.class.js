 ...
 * @namespace eventplanner.private
 */
...
eventplanner.private = {
     ...
     *      error : eventplanner.private.fn_error, // Callback en cas d'erreur
 ...
 */
eventplanner.private.handleAjaxErrorAPI = function(_request, _status, _error) {
	if (_request.readyState == 0) {
        return {type: 'AJAX', code: code, message: "Erreur réseau."};
    }
 ...
 */
eventplanner.private.getParamsAJAX = function(_params) {
    ...
    var paramsAJAX = {
        ...
        error: function(_request, _status, _error) {
            _params.error(eventplanner.private.handleAjaxErrorAPI(_request, _status, _error));
        },
...
// tests en console :
// try { eventplanner.private.checkParamsValue({value : [{test : 'check', test2 :'eeee'},{test : 'oefop', test2 : 'kfefe', test3 : 10}], regexp : /a|e|ch|1|zec/}); } catch(e) { console.log(e); }
eventplanner.private.checkParamValue = function(_params) {
    try {
 ...
 */
eventplanner.private.checkParamsRequired = function(_params, _paramsRequired) {
    var missings = Array();
    var group = Array();
 ...
 */
eventplanner.private.checkAndGetParams = function(_params, _paramsSpecifics, _paramsRequired) {
    // throw une exception en cas d'erreur (à attraper plus haut)
    eventplanner.private.checkParamsRequired(_params, _paramsRequired || []);

    // tout est ok, on merge avec les paramètres par défaut + les spécifiques à la fonction
    var params = $.extend({}, eventplanner.private.default_params, _paramsSpecifics, _params || {});
 ...
 */
eventplanner.private.checkParamsValue = function(_params) {
    if (Object.prototype.toString.call(_params) == '[object Object]') {
        eventplanner.private.checkParamValue(_params);
    } else {
        for (var i in _params) {
            eventplanner.private.checkParamValue(_params[i]);
        }
    }
};